...
import org.drsimonmiles.rocks.Move.allMoves

final case class Game (puzzle: Puzzle, x: Int, y: Int, boulders: Set[(Int, Int)], starRemains: Boolean) {
  ...
  def hasMan (mx: Int, my: Int): Boolean = x == mx && y == my
...
object Game {
  ...
  def canMove (game: Game, move: Move): Boolean = {
    import game._
    // Determines whether a boulder moved to (fx, fy) would squash the exit or star.
    def squashes (fx: Int, fy: Int): Boolean =
    ...
    def stillFalling = move != Fall && canMove (game, Fall)
    // Possible to move if the game is not already won, and
    //  the man is not trying to do anything but fall when there is no support below, and
    //  the man would not be squashed by the move, and
    //  the way is not blocked
    !hasWon && wouldntBeSquashed && !stillFalling && unblocked
  }

  /** For a boulder at (bx, by), let it fall until blocked by a floor or other boulder, return the resultant game state */
  private def drop (game: Game, bx: Int, by: Int): Game =
  ...
  def getAvailableMoves (game: Game): Seq[Move] =
    allMoves.filter (canMove (game, _))
  ...
  /** Sets the new position and removes the star from an initial game state if the player has moved onto it, returning the game state */
  def after (initial: Game, newX: Int, newY: Int): Game =
    initial.copy (x = newX, y = newY, starRemains = initial.starRemains && !initial.hasStar (newX, newY))
